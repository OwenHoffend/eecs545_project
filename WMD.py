...
def relaxed_WMD(doc1, doc2, word_library, w2v_model):
    nBOW1 = nBOW(doc1, word_library)
    nBOW2 = nBOW(doc2, word_library)
    C = w2v.get_C_mat(w2v_model, word_library)
    n = len(word_library)
...
def WMD_matrix(doc_words, word_library, w2v_model, wmd_func=WMD):
    n = len(doc_words)
    W = np.zeros((n, n))
    for i, d1 in enumerate(doc_words):
        for j, d2 in enumerate(doc_words):
            W[i, j] = wmd_func(d1, d2, word_library, w2v_model)
